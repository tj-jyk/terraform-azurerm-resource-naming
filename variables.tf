...
variable "workload" {
  ...
  validation {
    condition     = (var.workload == null || var.workload == "") ? true : can(regex("^[a-zA-Z0-9-]{1,16}$", var.workload))
...
variable "suffix" {
  ...
  validation {
    condition     = (var.suffix == null || var.suffix == "") ? true : can(regex("^[a-zA-Z0-9-]{1,16}$", var.suffix))
